...
class RecipeData:
    ...
    def to_nutrition_dict(self) -> dict:
        """
        Returns nutrition values as fractions (0-1) in the same format
        as ingredient_nutrition entries, derived from the recipe's own
        top-level proximate analysis fields.
        """
        def _safe(val, divisor=100.0):
            try:
                v = float(val)
                return v / divisor if v != 0 else 0.0
        ...
        # BETN = everything left on DM basis after CP, CF, fat, ash
        # Ash is not stored directly so we approximate:
        # BETN ≈ 100 - CP% - CF% - fat% - TDN_correction
        # Simpler: use NFE if present, otherwise back-calculate
        nfe = _safe(getattr(self, 'NFE', 0))
        if nfe > 0:
            betn = nfe
        else:
            # back-calculate on DM basis: BETN = 1 - CP - CF - fat - (1 - TDN) * fudge
            # conservative: just use what's left after known fractions
            betn = max(0.0, 1.0 - cp - cf - fat)
